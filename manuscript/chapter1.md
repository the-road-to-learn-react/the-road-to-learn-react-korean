...
자바스크립트 ES6에는 객체지향 프로그래밍에서 많이 사용되는 클래스(Class)문법이 도입되었습니다. 자바스크립트는 프로그래밍 철학에 있어 유연한 언어로써 함수형 프로그래밍과 객체지향 프로그래밍을 모두 적용할 수 있습니다.

리액트는 불변 자료구조나 다중 함수등 함수형 프로그래밍을 잘 활용하고 있으며, 컴포넌트를 만들 때는 ES6의 클래스를 활용합니다. 리액트는 두가지 프로그래밍 방식의 장점을 잘 사용하고 있습니다.

다음 Developer 클래스는 리액트 컴포넌트가 아닌, ES6 클래스입니다.
...
클래스에는 인스턴스화 할 수 있는 생성자(constructor)가 있습니다. 생성자는 매개변수를 클래스 인스턴스에 할당합니다. 클래스는 함수를 정의할 수 있습니다. 이런 함수는 클래스와 연관되어 있어 메서드(method) 혹은 클래스 메서드라고 부르기도 합니다. 

Developer 클래스는 클래스 선언문이고, 함수 호출을 통해 인스턴스들을 만들 수 있습니다. ES6 클래스 컴포넌트도 이와 비슷합니다. 선언이 필요하고, 어딘가에서 참조를 통해 인스턴스를 생성해야 합니다. 
...
리액트는 ES6 클래스 컴포넌트에 자바스크립트 ES6의 클래스를 사용합니다. 이미 한번은 보신 적이 있습니다.
...
App 컴포넌트를 만들 때 Component 클래스를 확장(extend)해서 만들었습니다. 확장은 객체 지향 프로그래밍에서 상속이라 부르는 개념과 관련이 있습니다. 한 클래스의 기능을 다른 클래스에서 사용하게 해주는 문법입니다. App 클래스는 Component 클래스를 확장함으로써, Component 클래스의 기능을 상속받습니다. Component 클래스는 ES6 클래스를 ES6 컴포넌트 클래스로 사용하려 할때 상속하는 클래스입니다. 여기에는 리액트의 컴포넌트로써 필요한 모든 기능이 들어있습니다. 그중에서 `render()`은 이미 사용했습니다. 앞으로 컴포넌트 클래스의 여러가지 메서드에 대해 배울 것입니다.

`Component` 클래스가 리액트 컴포넌트의 모든 구현 세부 사항을 캡슐화하고 있기 때문에 리액트에서 클래스를 컴포넌트로 사용할 수 있습니다. 

`Component` 클래스에서 외부에 노출된 것들은 인터페이스라고 부릅니다. 이중 한개의 메써드는 재작성해야하지만, 나머지는 안해도 됩니다. 자세한 사항은 리액트 생명주기(Life Cycle) 절에서 자세히 배울 것입니다. `render()` 메써드는 재작성해야만 하는데요. 이 함수가 리액트의 `Component` 의 출력을 제어하기 때문입니다. 이상이 ES6 클래스 기초와 이를 리액트 컴포넌트로 사용하는 기본적인 방법입니다.

### 읽어보기

* [리액트를 배우기전에 알아야 할 자바스크립트](https://www.robinwieruch.de/javascript-fundamentals-react-requirements/) 읽어보기
* [ES6 클래스](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Classes)
...
축하드립니다. 리액트 애플리케이션 작성 시작하기를 배웠습니다. 지금까지 학습한 내용을 정리해봅시다.

* 리액트 
    * *create-react-app*으로 리액트 애플리케이션 작성을 시작합니다. 
    * JSX 문법으로 HTML과 자바스크립트를 같이 사용해 컴포넌트의 결괏값을 만듭니다. 
    * 컴포넌트, 인스턴스, 엘레먼트는 서로 다릅니다. 
    * `ReactDOM.render()`는 리액트 애플리케이션이 리액트를 DOM에 연결시키는 진입점(entry point)입니다. 
...
이것으로 1장을 마칩니다. 지금까지 작성한 코드를 내 것으로 만드시기를 바랍니다. 이것저것 수정하며 테스트해보세요. 소스코드는 [공식 깃헙 리포](http://bit.ly/2H6Mfh3)에 있습니다.