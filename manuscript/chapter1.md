...
*src/index.js* 파일에 HMR(Hot Module Replacement)설정을 추가하면 개발환경이 더 편리해집니다. *create-react-app*을 사용하면 소스 코드가 바뀔때, 자동으로 브라우저가 리프레시됩니다. *src/App.js* 파일에서 `helloWorld` 변수를 변경해보면 브라우저가 리프레시되는 것을 볼 수 있습니다. 하지만, 소스 코드의 수정을 반영하는 더 좋은 방법이 있습니다.

HMR이란 페이지 리프레시를 하지 않으면서 애플리케이션을 다시 로딩해주는 도구입니다. *create-react-app*으로 생성된 소스 중 *src/index.js* 파일에 다음의 설정을 추가해서 활성화 할 수 있습니다.
...
이제 *src/App.js* 에 `helloWorld` 변수를 수정해봅시다. 페이지 리프레시는 하지 않으면서도 애플리케이션이 다시 로딩되고 제대로 출력됩니다. HMR에는 장점이 더 있습니다.

`console.log()`로 디버깅을 하는 중이었다면, 페이지 리프레시를 하지 않기때문에 콘솔에 적은 명령문들이 그대로 남아있게됩니다. 애플리케이션의 덩치가 커질 수록 페이지 리프레시가 생산성을 떨어트리게됩니다. HMR은 이로 인한 시간낭비를 줄여줍니다.

HMR의 가장 큰 장점은 재시동 후에도 애플리케이션의 상태가 유지되다는 점입니다. 애플리케이션에 3단계 대화창이 있다고 가정해봅시다. HMR이 없다면 소스 코드를 변경할 때마다 페이지가 리프레시되고, 1단계에서 3단계까지 다시 진행해야 합니다. HMR을 사용하면 다이얼로그 창이 3단계로 유지되며 리로딩 되기 전에 하고있던 디버깅을 그대로 할 수 있습니다. 이것이 페이지 리프레시 관련된 시간절약과 더불어 리액트 개발자에게 HMR이 편리한 이유입니다.

### 실습하기

* [이전 소스코드](http://bit.ly/2H6pn1j) 보기
  * [이 섹션에서 변경된 사항](http://bit.ly/2H8C6jW) 확인
* *src/App.js* 코드를 수정해 HMR이 어떻게 실행되는지 확인합니다.
* 댄 애브라몹(Dan Abramov)의 [리액트 라이브 : Hot Reloading으로 시간 여행 떠나기](https://www.youtube.com/watch?v=xsSnOQynTHs)를 10분간 시청합니다.
