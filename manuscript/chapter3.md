...
이 코드 속에 정말 많은 일들이 일어났습니다. 더 작은 조각으로 나눌까 싶었지만 그렇다면 각 조각 간 관계를 파악하기 어렵다고 판단했습니다. 단계별로 자세히 설명하겠습니다.
...
이를 위해 ES6 `Object.assign()` 메서드를 사용합니다. 첫 번째 인자에 타깃 객체이고, 나머지 인수는 소스 객체입니다. 이 객체를 병합해 타깃 객체에 병합합니다. 타깃 객체는 빈 객체가 될 수 있습니다. 소스 객체는 변경되지 않기 때문에 불변성 원칙을 고수합니다. 아래 코드를 작성해봅시다.
...
한 단계를 더 나아가 봅시다. 독립적인 조건부 렌더링에서 `result`에 의존하는 컴포넌트인 Table도 함께 포함시킵니다. `result`가 없더라도 모든 내용이 표시되어야 합니다. 이를 위해 JSX에서 삼항 조건 연산자(ternary operator)를 사용할 수 있습니다.
